...
def split_dataset(
    dataset: Dataset, task_idx: Union[List[int], int], num_tasks: int, split_strategy: str, tasks: list = None, split_seed : int = 42
):
    """
    Supports multiple task_idx s (passed as a list) for concatenated dataset for multiple tasks
    """
    if type(task_idx) == int:
        task_idx = [task_idx]
    if split_strategy == "class":
        assert len(dataset.classes) == sum([len(t) for t in tasks])
        masks = torch.stack([torch.tensor([(c in tasks[idx]) for c in dataset.targets]) for idx in task_idx])
        overall_mask = torch.any(masks, dim=0)
        indexes = overall_mask.nonzero()
        ...
        if len(task_idx) == 1:
            task_dataset = split_datasets[task_idx[0]]
        else:
            task_dataset = torch.utils.data.ConcatDataset([split_datasets[idx] for idx in task_idx])
...
def split_dataset_subset(
    dataset: Dataset, replay_task_idxs: List[int], num_tasks: int, split_strategy: str, tasks: list = None, split_seed : int = 42,
    proportion: float = 1.0, num_samples: int = None,
):
    """
    Supports multiple task_idx s (passed as a list) for concatenated dataset for multiple tasks
    """
    if len(replay_task_idxs) == 0:
        return None
    num_samples_per_task = num_samples // len(replay_task_idxs) if num_samples is not None else len(dataset)
    if split_strategy == "class":
        assert len(dataset.classes) == sum([len(t) for t in tasks])
        
        selected_indices = []
        for replay_task_idx in replay_task_idxs:
            num_samples_per_class = num_samples_per_task // len(tasks[replay_task_idx])
            for c in tasks[replay_task_idx]:
                class_samples_indices = torch.tensor([t == c for t in dataset.targets]).nonzero()
                selected_indices.append(
                    class_samples_indices[:min(num_samples_per_class, int(len(class_samples_indices) * proportion))]
                )
        indices = torch.concat(selected_indices)
        replay_dataset = Subset(dataset, indices)
    elif split_strategy == "data":
        assert tasks is None
        lengths = [len(dataset) // num_tasks] * num_tasks
        lengths[0] += len(dataset) - sum(lengths)
        ...
        partial_replay_datasets = []
        for replay_task_idx in replay_task_idxs:
            split_dataset = split_datasets[replay_task_idx]
            extracted_num_samples = min(num_samples_per_task, int(len(split_dataset) * proportion))
            partial_replay_datasets.append(Subset(split_dataset, np.arange(extracted_num_samples)))
        replay_dataset = torch.utils.data.ConcatDataset(partial_replay_datasets)
    elif split_strategy == "domain":
        assert tasks is None
        raise NotImplementedError
    return replay_dataset

def direct_prepare_split_dataset_subset(dataset: str,
    ...
    #dataset: Dataset, 
    replay_task_idxs: List[int]=None, num_tasks: int=0, split_strategy: str=None, tasks: list = None, split_seed : int = 42,
    proportion: float = 1.0, num_samples: int = None,
) -> Dataset:
    ...
    # First Create a dataset without transform - Direct data output
    ...
    if dataset in ["cifar10", "cifar100"]:
        ...
        dataset = dataset_with_index(ImageFolder)(train_dir, None) # task_transform)
    elif dataset == "custom":
        train_dir = data_dir / train_dir
        if no_labels:
        ...
        # data_dir は /content/.../wisdm2019_npy など、
        # train_X.npy / train_y.npy が置いてあるルートディレクトリを想定
        dataset = dataset_with_index(WISDMDataset)(
            data_dir,          # root= ではなく第一引数として渡す
            split="train",
            transform=None,    # transform は PreCachedDatasetWrapper 側で task_transform がかかる
        )
    
    clean_replay_dataset = split_dataset_subset(
        dataset=dataset, 
        replay_task_idxs=replay_task_idxs, 
        num_tasks=num_tasks,
        split_strategy=split_strategy, 
        tasks=tasks, 
        split_seed=split_seed,
        proportion=proportion, num_samples=num_samples,
    )
    
    return PreCachedDatasetWrapper(
        dataset=clean_replay_dataset,
...
class PreCachedDatasetWrapper(Dataset):
    def __init__(self, dataset, transform = None, target_transform=None):
        ...
        self.cached_dataset = []
        for data in dataset:
            self.cached_dataset.append(data)
        print("Dataset Cached")

    def __getitem__(self, index):
        data = self.cached_dataset[index]
        *other_info, sample, target = data

        if self.transform is not None:
            sample = self.transform(sample)
        if self.target_transform is not None:
            target = self.target_transform(target)
        return (*other_info, sample, target)
...
def prepare_datasets(
    ...
    if dataset in ["cifar10", "cifar100"]:
        ...
        online_eval_dataset = ImageFolder(data_dir / val_dir, transform=online_eval_transform)
...
def prepare_dataloader(
    train_dataset: Dataset, batch_size: int = 64, num_workers: int = 4, drop_last: bool = True
) -> DataLoader:
