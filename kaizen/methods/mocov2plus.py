...
from kaizen.methods.base import BaseMomentumModel
...
class MoCoV2Plus(BaseMomentumModel):
    ...
    def _dequeue_and_enqueue(self, keys: torch.Tensor):
        ...
        # Allow non divisible queue size
        if ptr + batch_size > self.queue_size:
            remaining_slots = self.queue_size - ptr
            self.queue[:, :, ptr : ptr + remaining_slots] = keys[:, :, :remaining_slots]
            self.queue[:, :, :batch_size - remaining_slots] = keys[:, :, remaining_slots:]
            ptr = batch_size - remaining_slots
        else:
