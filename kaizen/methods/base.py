...
from kaizen.utils.knn import WeightedKNNClassifier
from kaizen.utils.lars import LARSWrapper
from kaizen.utils.metrics import accuracy_at_k, weighted_mean
from kaizen.utils.momentum import MomentumUpdater, initialize_momentum_params
...
class BaseModel(pl.LightningModule):
    def __init__(
        ...
        lr: float,
        weight_decay: float,
        online_eval_classifier_lr: float,
        classifier_lr: float,
        ...
        knn_k: int = 20,
        classifier_training: bool = False,
        classifier_stop_gradient: bool = True,
        classifier_loss_weight: float = 1.,
        classifier_layers = [],
        online_evaluation_classifier_layers = [],
        **kwargs,
    ):
        ...
        self.online_evaluation_classifier_layers = online_evaluation_classifier_layers
        ...
        self.online_eval_classifier_lr = online_eval_classifier_lr
        # actual classifier args (new)
        self.classifier_training = classifier_training
        self.classifier_stop_gradient = classifier_stop_gradient
        ...
        if self.accumulate_grad_batches:
            ...
            self.online_eval_classifier_lr = self.online_eval_classifier_lr * self.accumulate_grad_batches
        ...
        if self.classifier_training:
            ...
            self.classifier = MultiLayerClassifier(self.features_dim, num_classes, self.classifier_layers)
        else:
    ...
    def add_model_specific_args(parent_parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument("--online_eval_classifier_lr", type=float, default=0.3)
        ...
        # classifier training
        parser.add_argument("--classifier_training", default=False, type=strtobool)
        parser.add_argument("--classifier_stop_gradient", default=True, type=strtobool)
        parser.add_argument("--classifier_loss_weight", default=1.0, type=float)
        parser.add_argument("--classifier_lr", type=float, default=0.3)
        parser.add_argument("--classifier_layers", type=int, default=[], nargs="*")

        # Online evaluation
        parser.add_argument("--online_evaluation", default=True, type=strtobool)
        parser.add_argument("--online_evaluation_training_data_source", choices=["all_tasks", "seen_tasks", "current_task"], default="all_tasks", type=str)
        parser.add_argument("--online_evaluation_classifier_layers", type=int, default=[], nargs="*")
    ...
    def learnable_params(self) -> List[Dict[str, Any]]:
        ...
        params = [
            ...
            {
                "name": "online_eval_classifier",
                "params": self.online_eval_classifier.parameters(),
                "lr": self.online_eval_classifier_lr,
                "weight_decay": 0,
            },
        ]
    ...
    def _online_eval_shared_step(self, X: torch.Tensor, targets) -> Dict:
        ...
        logits = self.online_eval_classifier(feats)
    ...
    def _classifier_partial_step(self, feats, targets) -> Dict:
        """Forwards a batch of images X and computes the classification loss, the logits, the
        features, acc@1 and acc@5.
    ...
    def training_step(self, batch: List[Any], batch_idx: int) -> Dict[str, Any]:
        ...
        if self.classifier_training:
            if self.classifier_stop_gradient:
                classifier_outs = [self._classifier_partial_step(feats.detach(), targets_task) for feats in outs_task["feats"]]
            else:
                classifier_outs = [self._classifier_partial_step(feats, targets_task) for feats in outs_task["feats"]]
            classifier_outs = {"classifier_" + k: [out[k] for out in classifier_outs] for k in classifier_outs[0].keys()}
            classifier_metrics = {
                f"train_{k}_crop_{i}": v for k in classifier_outs.keys() for i, v in enumerate(classifier_outs[k]) if "logits" not in k
            }
            self.log_dict(classifier_metrics, on_epoch=True, sync_dist=True)
            loss_classifier_1, loss_classifier_2 = classifier_outs.pop("classifier_loss")
            loss += self.classifier_loss_weight * (loss_classifier_1 + loss_classifier_2)
        else:
            classifier_outs = {}
        ...
        if self.online_eval:
            ...
            *_, X_online_eval, targets_online_eval = batch["online_eval"]
            ...
            online_eval_loss = outs_online_eval.pop("online_eval_loss")

            loss += online_eval_loss
        else:
            outs_online_eval = {}
        outs = {**outs_task, **outs_online_eval, **classifier_outs, **{"loss": loss}}
        return outs
    ...
    def validation_epoch_end(self, outs: List[Dict[str, Any]]):
        ...
        log = {}
        if self.classifier_training:
            val_loss = weighted_mean(outs, "val_classifier_loss", "batch_size")
            val_acc1 = weighted_mean(outs, "val_classifier_acc1", "batch_size")
            val_acc5 = weighted_mean(outs, "val_classifier_acc5", "batch_size")
            val_loss = torch.mean(self.all_gather(val_loss))
            val_acc1 = torch.mean(self.all_gather(val_acc1))
            val_acc5 = torch.mean(self.all_gather(val_acc5))

            log.update({"val_classifier_loss": val_loss, "val_classifier_acc1": val_acc1, "val_classifier_acc5": val_acc5})

            # if not self.trainer.sanity_checking:
            if True:
                preds = torch.cat([o["classifier_logits"].max(-1)[1] for o in outs]).cpu().numpy()
                ...
                if self.split_strategy == "class":
                    ...
                    for task_idx, task in enumerate(self.tasks):
                        ...
                        log[f"val_classifier_acc1_task{task_idx}"] = correct_task / mask_task.sum()
                ...
                if self.split_strategy == "domain":
                    ...
                    for task_idx, domain in enumerate(self.domains):
                        ...
                        log[f"val_classifier_acc1_{domain}_{task_idx}"] = correct_domain / mask_domain.sum()

        if self.online_eval:
            val_loss = weighted_mean(outs, "val_online_eval_loss", "batch_size")
            val_acc1 = weighted_mean(outs, "val_online_eval_acc1", "batch_size")
            val_acc5 = weighted_mean(outs, "val_online_eval_acc5", "batch_size")
            val_loss = torch.mean(self.all_gather(val_loss))
            val_acc1 = torch.mean(self.all_gather(val_acc1))
            val_acc5 = torch.mean(self.all_gather(val_acc5))


            log.update({"val_online_eval_loss": val_loss, "val_online_eval_acc1": val_acc1, "val_online_eval_acc5": val_acc5})

            # if not self.trainer.sanity_checking:
            if True:
                preds = torch.cat([o["online_eval_logits"].max(-1)[1] for o in outs]).cpu().numpy()
                targets = torch.cat([o["targets"] for o in outs]).cpu().numpy()
                mask_correct = preds == targets
                ...
                if self.split_strategy == "domain":
                    ...
                    for task_idx, domain in enumerate(self.domains):
                        mask_domain = np.isin(domains, np.array([domain]))
                        correct_domain = np.logical_and(mask_domain, mask_correct).sum()
                        log[f"val_online_eval_acc1_{domain}_{task_idx}"] = correct_domain / mask_domain.sum()
        ...
        if self.online_eval:
            ...
            *_, X, targets = batch
            ...
            if self.momentum_classifier is not None:
                metrics = {
                    "batch_size": batch_size,
                    "val_momentum_loss": out["loss"],
                    "val_momentum_acc1": out["acc1"],
                    "val_momentum_acc5": out["acc5"],
                }
    ...
    def validation_epoch_end(self, outs: Tuple[List[Dict[str, Any]]]):
        ...
        if self.online_eval:
            ...
            if self.momentum_classifier is not None:
                ...
                val_loss = weighted_mean(momentum_outs, "val_momentum_loss", "batch_size")
                val_acc1 = weighted_mean(momentum_outs, "val_momentum_acc1", "batch_size")
                val_acc5 = weighted_mean(momentum_outs, "val_momentum_acc5", "batch_size")

                log = {
                    "val_momentum_loss": val_loss,
                    "val_momentum_acc1": val_acc1,
                    "val_momentum_acc5": val_acc5,
                }
                self.log_dict(log, sync_dist=True)
